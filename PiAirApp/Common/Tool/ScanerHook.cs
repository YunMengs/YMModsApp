    ...
    public class ScanerHook
    {
        ...
        public bool Start()
        {
            if (hKeyboardHook == 0)
            {
                ...
                GCHandle.Alloc(hKeyboardHook);///！！！！！阻止回收，否则必定出错
            }
        ...
        public bool Stop()
        {
            if (hKeyboardHook != 0)
            {
                bool b = UnhookWindowsHookEx(hKeyboardHook);
                hKeyboardHook = 0;
                return b;
            }
            return true;
        ...
        private int KeyboardHookProc(int nCode, Int32 wParam, IntPtr lParam)
        {
            ...
            codes.Add(msg);
            if (ScanerEvent != null && (msg.message == 13 || msg.message == 9) && msg.paramH > 0 && !string.IsNullOrEmpty(codes.Result))
            {
                ScanerEvent(codes.Clone());
                msg.message = 0;
                msg.paramH = 0;
                codes.Result = "";
            }
        ...
        public class ScanerCodes : ICloneable
        {
            public string Name;
            ...
            public ScanerCodes Clone()
            {
                return (ScanerCodes)this.MemberwiseClone();
            }
            ...
            public void Add(EventMsg msg)
            {
                ...
                #region 获取键盘状态
                // 记录正在按下的按键
                if ((msg.paramH == 0 || msg.paramH == 1) && !_keydown.Contains(msg.message))
                {
                    _keydown.Add(msg.message);
                }
                // 清除已松开的按键
                if (msg.paramH > 1 && _keydown.Contains(msg.message))
                {
                ...
                if (GetKeyNameText(c * 65536, strKeyName, 255) > 0)
                {
                    ...
                    if (_key.Length == 1 && msg.paramH == 0)
                    {
                        ...
                        _cur = ShiftChar(_key, isShift, isRightShift, _state).ToString();
                        if (_result.Count != 0)
                        {
            ...
            private char ShiftChar(string k, bool isShiftDown, bool isRightShiftDown, byte[] state)
            {
                ...
                if (isShiftDown || isRightShiftDown)
                {
                    if (chr >= 'a' && chr <= 'z')
                    {
                        chr = (char)((int)chr - 32);
